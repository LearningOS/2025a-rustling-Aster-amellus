    ...
    T: Default,
{
    ...
    fn smallest_child_idx(&self, idx: usize) -> usize {
        // Choose the child with higher priority according to comparator
    ...
    T: Default,
{
    ...
    fn next(&mut self) -> Option<T> {
        ...
        // Move last element to root (if any) and sift down
        ...
        if self.count >= 1 {
            ...
            while self.children_present(i) {
                let c = self.smallest_child_idx(i);
                if (self.comparator)(&self.items[c], &self.items[i]) {
                    self.items.swap(i, c);
                    i = c;
                } else {
                    break;
